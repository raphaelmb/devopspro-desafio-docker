...
[Link do repositório da aplicação Node.js](https://github.com/raphaelmb/conversao-temperatura)

[Link do repositório da aplicação Python](https://github.com/raphaelmb/conversao-distancia)

[Link do repositório da aplicação .NET](https://github.com/raphaelmb/conversao-peso)

# QUESTÃO 4

Foi adicionado um arquivo docker-compose.yaml ao projeto para subir a aplicação e banco de dados com o comando `docker-compose up`. Neste arquivo os comandos estão dispostos de forma declarativa: criação de rede, volumes e containers (com suas especificidades como variáveis de ambiente, portas, imagem, etc.).
...
A partir deste docker-compose.yaml, roda-se um container Wordpress e container do banco de dados MySQL.
...
O presente docker-compose.yaml sobe uma aplicação baseada em microsserviços. Foram criadas duas redes pois o serviço de "review" e "movie" se comunicam somente com a aplicação "rotten-potatoes" e não entre si, não havendo necessidade de estarem em uma mesma rede. As imagens das aplicações utilizadas estão hosteadas no DockerHub.
...
[Link do repositório](https://github.com/raphaelmb/rotten-potatoes-ms)
